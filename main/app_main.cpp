...
struct AppContext {
  ...
  std::uint8_t led_brightness_percent =
      ai_keyboard::kWs2812BrightnessPercent;
  std::uint8_t saved_led_brightness_percent =
      ai_keyboard::kWs2812BrightnessPercent;
...
void apply_music_mode_result(AppContext* app,
                             ...
                             std::uint32_t now_ms) {
  if (app == nullptr || result.action == ai_keyboard::MusicModeAction::None) {
    return;
  }
  switch (result.action) {
    case ai_keyboard::MusicModeAction::EnterMusic: {
#if defined(EASY_INPUT_MUSIC_PLAYER)
      if (!app->music_player_ready) {
        app->music_mode.reset();
        app->leds.set_music_visual(false, false, now_ms);
        ESP_LOGE(kTag, "offline music mode rejected: player unavailable");
        return;
      }
      app->music_player.set_volume_percent(result.volume_percent);
#else
      app->music_mode.reset();
      app->leds.set_music_visual(false, false, now_ms);
      ESP_LOGW(kTag, "offline music mode is not present in this build");
      return;
#endif
      app->encoder_press_gesture.reset();
      app->led_brightness_mode.reset();
      app->leds.end_brightness_preview();
      app->leds.set_music_visual(true, false, now_ms);
      ESP_LOGI(kTag, "offline music mode entered volume=%u%%",
               static_cast<unsigned>(result.volume_percent));
      return;
    }
    case ai_keyboard::MusicModeAction::ExitMusic:
#if defined(EASY_INPUT_MUSIC_PLAYER)
      ...
      app->leds.end_brightness_preview();
      app->leds.set_music_visual(false, false, now_ms);
      ESP_LOGI(kTag, "music mode exited");
      return;
    case ai_keyboard::MusicModeAction::TogglePause:
#if defined(EASY_INPUT_MUSIC_PLAYER)
      app->music_player.set_paused(app->music_mode.paused());
#endif
      app->leds.set_music_visual(true, app->music_mode.paused(), now_ms);
      ESP_LOGI(kTag, "music playback %s",
               app->music_mode.paused() ? "paused" : "resumed");
      return;
    case ai_keyboard::MusicModeAction::PlayOrNext:
#if defined(EASY_INPUT_MUSIC_PLAYER)
      if (app->music_player_ready && app->music_player.play_or_next()) {
        app->leds.set_music_visual(true, false, now_ms);
        ESP_LOGI(kTag, "offline music play-or-next queued=1");
      } else {
        ESP_LOGE(kTag, "offline music play-or-next queued=0");
      }
#else
      ESP_LOGW(kTag, "offline music player is not present in this build");
#endif
      return;
    case ai_keyboard::MusicModeAction::VolumeChanged:
#if defined(EASY_INPUT_MUSIC_PLAYER)
      app->music_player.set_volume_percent(result.volume_percent);
#endif
      ESP_LOGI(kTag, "music volume=%u%%",
               static_cast<unsigned>(result.volume_percent));
      return;
    case ai_keyboard::MusicModeAction::None:
      return;
  }
}
...
void service_music(AppContext* app) {
  ...
  std::uint16_t rms = 0;
  std::uint16_t beat = 0;
  if (app->music_player.take_visual(&rms, &beat)) {
    app->leds.set_music_audio_level(rms, beat, millis());
  }
#else
...
void apply_led_brightness_action(
    AppContext* app,
    const ai_keyboard::LedBrightnessAction& action) {
  using Kind = ai_keyboard::LedBrightnessActionKind;
  switch (action.kind) {
    case Kind::None:
      return;
    case Kind::DispatchClick:
      dispatch_encoder_press_click(app);
      return;
    case Kind::Entered:
      app->led_brightness_percent = action.brightness_percent;
      app->input_led_feedback_pending = false;
      app->leds.set_brightness_percent(action.brightness_percent);
      app->leds.show_brightness_preview();
      ESP_LOGI(kTag,
               "LED brightness mode entered percent=%u",
               static_cast<unsigned>(action.brightness_percent));
      return;
    case Kind::Changed:
      app->led_brightness_percent = action.brightness_percent;
      app->leds.set_brightness_percent(action.brightness_percent);
      ESP_LOGI(kTag,
               "LED brightness preview percent=%u",
               static_cast<unsigned>(action.brightness_percent));
      return;
    case Kind::Saved: {
      esp_err_t save_err = ESP_OK;
      if (!app->config_store.save_led_brightness(
              action.brightness_percent, &save_err)) {
        app->led_brightness_percent = app->saved_led_brightness_percent;
        app->leds.set_brightness_percent(app->saved_led_brightness_percent);
        app->leds.end_brightness_preview();
        app->leds.show_status_event(
            easy_input::StatusLedEvent::SaveFailed, millis());
        ESP_LOGW(kTag,
                 "LED brightness save failed; restored percent=%u: %s",
                 static_cast<unsigned>(app->saved_led_brightness_percent),
                 esp_err_to_name(save_err));
        return;
      }
      app->led_brightness_percent = action.brightness_percent;
      app->saved_led_brightness_percent = action.brightness_percent;
      app->leds.set_brightness_percent(action.brightness_percent);
      app->leds.end_brightness_preview();
      ESP_LOGI(kTag,
               "LED brightness saved percent=%u key=led_brt_v1",
               static_cast<unsigned>(action.brightness_percent));
      return;
    }
    case Kind::Cancelled:
      app->encoder_press_gesture.reset();
      app->led_brightness_mode.cancel_click_sequence();
      app->led_brightness_percent = action.brightness_percent;
      app->leds.set_brightness_percent(action.brightness_percent);
      app->leds.end_brightness_preview();
      ESP_LOGI(kTag,
               "LED brightness mode cancelled percent=%u",
               static_cast<unsigned>(action.brightness_percent));
      return;
  }
}
...
ai_keyboard::PowerPolicyInputs power_policy_inputs(AppContext* app,
  ...
  inputs.encoder_press_pending = app->encoder_press_gesture.pending() ||
                                 app->led_brightness_mode.interaction_pending() ||
                                 app->encoder_text_selection_exit_pending ||
                                 app->encoder_text_selection_chord_pending ||
                                 !app->pending_encoder_text_selection_steps.empty();
...
#if defined(EASY_INPUT_MUSIC_PLAYER)
  inputs.speaker_playback_active =
      inputs.speaker_playback_active || app->music_player.sleep_blocked();
#endif
...
void check_encoder_press_config_hold(AppContext* app,
  ...
  if (app->led_brightness_mode.interaction_pending() ||
      !app->encoder_press_gesture.trigger_config_if_due(
          observed_ms,
...
void service_speaker(AppContext* app) {
  ...
  bool power_required = app->speaker.power_lease_required();
#if defined(EASY_INPUT_MUSIC_PLAYER)
  power_required = power_required || app->music_player.power_required();
#endif
  ...
  if (app->speaker.power_lease_required() &&
      power_required && app->peripheral_power.ready()) {
...
bool handle_input_event(const easy_input::InputEvent& event, void* context) {
  ...
  if (music_result.action != ai_keyboard::MusicModeAction::None) {
    apply_music_mode_result(app, music_result, now);
  }
  if (music_result.consumed) {
    // The active music mode and both edges of its exit gesture exclusively own
    // S1-S8 and the encoder, so no event can leak into HID/PTT/brightness.
    return true;
  }
  // The mode-switch double click is consumed here; ordinary slower clicks
  // remain available to the LED brightness state machine.
  if (app->music_mode.suppress_encoder_release()) {
    return true;
  }
  ...
  if (encoder_turn) {
    if (app->led_brightness_mode.active()) {
      const int magnitude = encoder_step_count(event.encoder_step);
      const int brightness_delta = event.encoder_step < 0
                                       ? magnitude
                                       : -magnitude;
      apply_led_brightness_action(
          app,
          app->led_brightness_mode.rotate(brightness_delta, now));
      return true;
    }
  ...
  if (event.input == ai_keyboard::InputId::EncoderPress) {
    if (event.phase == ai_keyboard::InputPhase::Pressed) {
      apply_led_brightness_action(
          app, app->led_brightness_mode.press(event.timestamp_ms));
    ...
    if (release.dispatch_click) {
      apply_led_brightness_action(
          app,
          app->led_brightness_mode.short_release(
              event.timestamp_ms, app->led_brightness_percent));
...
ai_keyboard::AwakeWaitDecision plan_next_awake_work(AppContext* app,
  ...
  planner.add_deadline(
      ...
      "encoder_hold");
  deadline_ms = 0;
  const bool brightness_deadline =
      app->led_brightness_mode.next_deadline_ms(&deadline_ms);
  planner.add_deadline(
      brightness_deadline, deadline_ms, "led_brightness_mode");
  deadline_ms = 0;
...
extern "C" void app_main(void) {
  ...
  if (!app.config_store.load_led_brightness(
    ...
    app.led_brightness_percent = ai_keyboard::kWs2812BrightnessPercent;
    if (brightness_load_err != ESP_ERR_NVS_NOT_FOUND) {
      ESP_LOGW(kTag,
               "LED brightness load failed; using default=%u: %s",
               static_cast<unsigned>(app.led_brightness_percent),
               esp_err_to_name(brightness_load_err));
    }
  }
  app.saved_led_brightness_percent = app.led_brightness_percent;
  app.leds.set_brightness_percent(app.led_brightness_percent);
  ...
  app.music_player_ready = music_err == ESP_OK;
  if (music_err != ESP_OK) {
    ESP_LOGW(kTag, "offline music player unavailable: %s",
  ...
  while (true) {
    ...
    check_encoder_press_config_hold(
        ...
        app.inputs.low_active_pressed(ai_keyboard::kEncoderPressPin));
    apply_led_brightness_action(
        &app, app.led_brightness_mode.update(millis()));
