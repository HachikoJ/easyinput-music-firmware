  ...
  switch (event.kind) {
    ...
    case ai_keyboard::FirmwareEventKind::AppCommand:
      if (event.host_action) {
        const auto wire = ai_keyboard::encode_host_action_v1(event.value);
        if (!wire.has_value()) {
          ESP_LOGW(kTag, "USB APP_COMMAND host_action rejected");
          return false;
        }
        return send_app_command_report(
            ...
            wire->chunk_index,
            wire->total_chunks,
            reinterpret_cast<const std::uint8_t*>(wire->data.data()),
            wire->data_length,
            expected_epoch);
      }
      return false;
