...
static_assert(ai_keyboard::kWs2812Count ==
              ai_keyboard::kLedBrightnessPreviewColors.size());
...
void StatusLedStrip::set_brightness_percent(
    std::uint8_t brightness_percent) {
  brightness_percent_ =
      ai_keyboard::clamp_ws2812_brightness_percent(brightness_percent);
  if (brightness_preview_active_) {
    show_brightness_preview();
  }
}

void StatusLedStrip::show_brightness_preview() {
  brightness_preview_active_ = true;
  cold_boot_sequence_.cancel();
  deferred_feedback_.clear();
  active_feedback_ = {};
  effect_until_ms_ = 0;
  ...
  for (std::size_t index = 0; index < leds_.size(); ++index) {
    const auto color = ai_keyboard::kLedBrightnessPreviewColors[index];
    leds_[index] = {color.red, color.green, color.blue};
  }
  if (ready()) {
    flush();
  }
...
void StatusLedStrip::end_brightness_preview() {
  brightness_preview_active_ = false;
  active_feedback_ = {};
  effect_until_ms_ = 0;
  last_frame_ms_ = 0;
  idle_rendered_ = false;
...
void StatusLedStrip::set_agent_status(const ai_keyboard::AgentStatusCommand& command,
  ...
  idle_rendered_ = false;
  if (!brightness_preview_active_ &&
      !active_feedback_.active && !cold_boot_sequence_.active()) {
...
void StatusLedStrip::set_music_visual(bool active,
                                       bool paused,
                                       std::uint32_t now_ms) {
  const bool was_active = music_visual_active_;
  music_visual_active_ = active;
  music_visual_paused_ = paused;
  if (!active) {
    music_rms_milli_ = 0;
    music_beat_milli_ = 0;
    music_color_phase_ = 0;
    idle_rendered_ = false;
    return;
  }
  if (!was_active) {
  ...
  effect_until_ms_ = 0;
  idle_rendered_ = false;
  music_last_frame_ms_ = now_ms - 30U;
  update(now_ms);
}

void StatusLedStrip::set_music_audio_level(std::uint16_t rms_milli,
                                           std::uint16_t beat_milli,
                                           std::uint32_t) {
  if (!music_visual_active_ || music_visual_paused_) {
    return;
  }
...
void StatusLedStrip::show_feedback(const ai_keyboard::InputActivityFeedback& feedback,
  ...
  if (!feedback.active || brightness_preview_active_) {
    return;
  }
...
void StatusLedStrip::update(std::uint32_t now_ms) {
  ...
  if (music_visual_active_) {
    if (music_visual_paused_) {
      if (!idle_rendered_) {
        for (std::size_t index = 0; index < leds_.size(); ++index) {
          leds_[index] = scale_rgb(
              rainbow_color(static_cast<std::uint8_t>(index)), 1U, 5U);
        }
        flush();
        idle_rendered_ = true;
      }
    } else {
      if (now_ms - music_last_frame_ms_ < 30U) {
        return;
      }
      const auto level = static_cast<std::uint16_t>(
          std::min<std::uint32_t>(1000U,
      220U + (static_cast<std::uint32_t>(music_rms_milli_) * 430U) / 1000U +
              (static_cast<std::uint32_t>(music_beat_milli_) * 480U) / 1000U));
      // Keep the idle music state visible at the configured 35% global LED
      // brightness, then let RMS/beat energy drive the breathing amplitude.
      const auto value = static_cast<std::uint8_t>((level * 96U) / 1000U);
      const auto beat_phase = music_beat_milli_ >= 350U
                                  ? 2U
                                  : music_beat_milli_ >= 150U ? 1U : 0U;
      music_color_phase_ = static_cast<std::uint8_t>(
          (now_ms / 180U + beat_phase) % 7U);
      for (std::size_t index = 0; index < leds_.size(); ++index) {
        leds_[index] = scale_rgb(
            rainbow_color(static_cast<std::uint8_t>(
                music_color_phase_ + static_cast<std::uint8_t>(index))),
            value,
            47U);
      }
      flush();
...
esp_err_t StatusLedStrip::flush() {
  ...
  for (std::size_t index = 0; index < leds_.size(); ++index) {
    ...
    encode_ws2812_byte(
        ai_keyboard::scale_ws2812_channel(color.green, brightness_percent_), pixel);
    encode_ws2812_byte(
        ai_keyboard::scale_ws2812_channel(color.red, brightness_percent_), pixel + 8U);
    encode_ws2812_byte(
        ai_keyboard::scale_ws2812_channel(color.blue, brightness_percent_), pixel + 16U);
  }
