...
std::string build_speaker_probe_status_json(
  ...
  append_bounded_string_field(
      out, &first, "firmware", snapshot.firmware, 16, "unknown");
  append_bounded_string_field(
...
std::string build_config_status_json(const ConfigStatusSnapshot& snapshot) {
  ...
  // Keep current power truth when recent cycle evidence is the part that
  // exceeds the 512-byte envelope; omit only optional cycle_* fields first.
  const auto compact_without_cycle = build_compact_status_json(
      snapshot,
      preserve_fingerprint,
      false,
      true,
