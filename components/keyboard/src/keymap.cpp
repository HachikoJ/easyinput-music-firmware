...
FirmwareEvent host_action_event(const std::string& value, InputPhase phase) {
  if (phase == InputPhase::Released) return {};
  constexpr std::string_view kPrefix = "host_action:";
  if (value.compare(0, kPrefix.size(), kPrefix) != 0) return {};
  FirmwareEvent event{FirmwareEventKind::AppCommand,
                      value.substr(kPrefix.size())};
  event.host_action = true;
  return event;
}
...
FirmwareEvent event_for_action(const Action& action,
  ...
  switch (action.kind) {
    ...
    case ActionKind::HostAction:
      return host_action_event(action.text, phase);
