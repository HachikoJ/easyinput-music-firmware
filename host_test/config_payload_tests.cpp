...
void parses_host_action_binding_with_full_storage_prefix() {
  const auto result = ai_keyboard::parse_config_payload(R"({
    "schema":"ai_keyboard.v1",
    "profiles":[{"id":"default","keys":{
      "KEY1":{"press":"disabled"},
      "KEY2":{"press":"host_action:123e4567-e89b-12d3-a456-426614174000"},
      "KEY3":{"press":"disabled"},"KEY4":{"press":"disabled"},
      "KEY5":{"press":"disabled"},"KEY6":{"press":"disabled"},
      "KEY7":{"press":"disabled"},"KEY8":{"press":"disabled"}
    },"encoder":{"left":"disabled","right":"disabled","press":"disabled"}}]
  })");
  assert(result.status == ConfigParseStatus::Ok);
  const auto& action = result.config.keymap.action_for(InputId::Key2);
  assert(action.kind == ActionKind::HostAction);
  assert(action.text == "host_action:123e4567-e89b-12d3-a456-426614174000");
}

void rejects_noncanonical_host_action_uuid_without_normalizing() {
  const std::vector<std::string> invalid{
  ...
  for (const auto& value : invalid) {
    auto payload = R"({
      "schema":"ai_keyboard.v1","profiles":[{"id":"default","keys":{
        "KEY1":{"press":"disabled"},"KEY2":{"press":")" + value + R"("},
        "KEY3":{"press":"disabled"},"KEY4":{"press":"disabled"},
        "KEY5":{"press":"disabled"},"KEY6":{"press":"disabled"},
        "KEY7":{"press":"disabled"},"KEY8":{"press":"disabled"}
      },"encoder":{"left":"disabled","right":"disabled","press":"disabled"}}]
    })";
    assert(ai_keyboard::parse_config_payload(payload).status ==
           ConfigParseStatus::UnknownAction);
  }
}

void all_eight_keys_preserve_host_action_prefix_and_emit_press_only() {
  constexpr const char* kUuid =
      "123e4567-e89b-12d3-a456-426614174000";
  std::string payload = R"({
    "schema":"ai_keyboard.v1",
    "profiles":[{"id":"default","keys":{
      "KEY1":{"press":"host_action:UUID"},
      "KEY2":{"press":"host_action:UUID"},
      "KEY3":{"press":"host_action:UUID"},
      "KEY4":{"press":"host_action:UUID"},
      "KEY5":{"press":"host_action:UUID"},
      "KEY6":{"press":"host_action:UUID"},
      "KEY7":{"press":"host_action:UUID"},
      "KEY8":{"press":"host_action:UUID"}
    },"encoder":{"left":"disabled","right":"disabled","press":"disabled"}}]
  })";
  const std::string needle = "host_action:UUID";
  const std::string replacement = std::string("host_action:") + kUuid;
  std::size_t offset = 0;
  while ((offset = payload.find(needle, offset)) != std::string::npos) {
    payload.replace(offset, needle.size(), replacement);
    offset += replacement.size();
  }
  const auto result = ai_keyboard::parse_config_payload(payload);
  assert(result.status == ConfigParseStatus::Ok);

  const std::string stored = std::string("host_action:") + kUuid;
  const std::array<InputId, 8> keys{{
  ...
  for (const auto key : keys) {
    ...
    assert(action.kind == ActionKind::HostAction);
    assert(action.text == stored);
    const auto press = ai_keyboard::event_for_action(
        action, ai_keyboard::InputPhase::Pressed, "", "", ai_keyboard::HostPlatform::MacOS);
    const auto release = ai_keyboard::event_for_action(
        action, ai_keyboard::InputPhase::Released, "", "", ai_keyboard::HostPlatform::MacOS);
...
int main() {
  ...
  parses_host_action_binding_with_full_storage_prefix();
  rejects_noncanonical_host_action_uuid_without_normalizing();
  all_eight_keys_preserve_host_action_prefix_and_emit_press_only();
  return 0;
}
