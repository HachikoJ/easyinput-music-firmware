...
void assert_sync_core(const std::string& json,
  ...
  const auto expected_capabilities = !legacy_capabilities
      ...
      : (usb_management
             ? R"("capabilities":{"semantic_actions":true,"offline_platform_switch":true,"config_max_bytes":2048,"host_action_v1":true,"usb_management_v1":true})"
...
void assert_host_action_capability_once(const std::string& json) {
...
void builds_compact_status_json() {
  ...
  assert_host_action_capability_once(json);
}
...
void carries_truthful_recent_deep_sleep_cycle_when_budget_allows() {
  ...
  assert(json.find(R"("deep_sleep_block":"k")") !=
         std::string::npos);
...
void speaker_probe_worst_case_is_versioned_complete_and_gatt_safe() {
  ai_keyboard::ConfigStatusSnapshot snapshot;
  snapshot.firmware = std::string(30, 'f');
...
void status_variants_publish_boolean_host_action_and_ble_budget() {
  ai_keyboard::ConfigStatusSnapshot snapshot;
  ...
  assert_host_action_capability_once(confirmation);
  assert(confirmation.size() <= ai_keyboard::kConfigStatusGattSafeLen);

  snapshot.phase = "battery";
  const auto battery = ai_keyboard::build_config_status_json(snapshot);
  assert_host_action_capability_once(battery);
  const auto battery_ble = ai_keyboard::append_ble_status_wire_json(
      battery, {true, true, UINT16_MAX, UINT16_MAX, UINT16_MAX});
  assert_host_action_capability_once(battery_ble);
  assert(battery_ble.size() <= ai_keyboard::kConfigStatusGattSafeLen);
  ...
  assert_host_action_capability_once(probe);
